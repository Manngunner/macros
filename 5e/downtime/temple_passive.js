...
const maxSermonBase = new Intl.NumberFormat("default", {
  minimumFractionDigits: 0,
  maximumFractionDigits: 0,
}).format(+totalSeating * 20);
const minSermonBase = new Intl.NumberFormat("default", {
  minimumFractionDigits: 0,
  maximumFractionDigits: 0,
}).format(+totalSeating * 0.2);
...
<form>
  ...
  <ul style="list-style-type:none;">
    <li>Free workers: ${new Intl.NumberFormat("default", {
      minimumFractionDigits: 0,
      maximumFractionDigits: 1,
    }).format(+slaveLabour * +slavePay)}gp</li>
    <li>Paid workers: ${new Intl.NumberFormat("default", {
      minimumFractionDigits: 0,
      maximumFractionDigits: 1,
    }).format(+wageLabour * +wagePay)}gp</li>
  ...
  <ul style="list-style-type:none;">
    ...
    <li>Local popularity: ${new Intl.NumberFormat("default", {
      style: "percent",
      minimumFractionDigits: 0,
      maximumFractionDigits: 0,
    }).format(localPopularity)}</li>
    <li>Local wealth: ${new Intl.NumberFormat("default", {
      style: "percent",
      minimumFractionDigits: 0,
      maximumFractionDigits: 0,
    }).format(localWealth)}</li>
  </ul>
</form>
`;
...
async function main(days, pcSermon) {
  ...
  const totalSlavePay = new Intl.NumberFormat("default", {
    minimumFractionDigits: 0,
    maximumFractionDigits: 1,
  }).format(+slaveLabour * +slavePay * +days);
  const totalWagePay = new Intl.NumberFormat("default", {
    minimumFractionDigits: 0,
    maximumFractionDigits: 1,
  }).format(+wageLabour * +wagePay * +days);
  ...
  if (totalSermonPerformed > 0) {
    for (let iter = 0; iter < totalSermonPerformed; iter++) {
      ...
      const minimumSermonIncome = new Intl.NumberFormat("default", {
        minimumFractionDigits: 0,
        maximumFractionDigits: 2,
      }).format(+minSermonBase * +localPopularity * +localWealth);
      // Define the roll for the maximum sermon income
      const maximumSermonIncome = new Intl.NumberFormat("default", {
        minimumFractionDigits: 0,
        maximumFractionDigits: 0,
      }).format(+maxSermonBase * +localPopularity * +localWealth);
      if (pcSermon === true) {
        const religiousroll = await new Roll(`1d20+${relmod}`).evaluate({
          async: true,
        });
        const persuasionroll = await new Roll(`1d20+${permod}`).evaluate({
          async: true,
        });
      ...
      const incomeRoll = await new Roll(
        `${sermonSkill}d${maximumSermonIncome}kh+${minimumSermonIncome}`
      ).evaluate({ async: true });
      ...
      if (incomeRoll.total - minimumSermonIncome >= maximumSermonIncome / 2) {
        localPopularity =
          ((incomeRoll.total - minimumSermonIncome) / maximumSermonIncome) *
            maxSwing +
          localPopularity;
        // Below the average will have a negative effect on popularity
      } else if (
        incomeRoll.total - minimumSermonIncome <
        maximumSermonIncome / 2
      ) {
        localPopularity =
          localPopularity -
          ((incomeRoll.total - minimumSermonIncome) / maximumSermonIncome) *
  ...
  const totalIncome = new Intl.NumberFormat("default", {
    minimumFractionDigits: 0,
    maximumFractionDigits: 2,
  }).format(+totalSermonIncome - (+totalSlavePay + +totalWagePay));
  const newPopularity = new Intl.NumberFormat("default", {
    style: "percent",
    minimumFractionDigits: 0,
    maximumFractionDigits: 0,
  }).format(localPopularity);
  const chatMsg = `
    <p>Days: ${days}</p>
