...
async function main() {
  ...
  const medicineMsg = formatmessage(medicineroll.total, medicinecontest.total);
  const natureMsg = formatmessage(natureroll.total, naturecontest.total);
  const survivalMsg = formatmessage(survivalroll.total, survivalcontest.total);
  chatMsg += `
    <details closed="">
        <h3>Medicine test</h3>
        ${medicineMsg}
